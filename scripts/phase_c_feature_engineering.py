...
What this does, biologically/chemically:
  1. Splits the lipid_molar_ratio string ("60:20:19.5:0.5") using the
     official LNP Atlas order: ionizable / PEG-lipid / sterol / helper.
     Raw reported ratio values are preserved, and separately normalized
     percentages are calculated so they sum to 100. This is the
...
def parse_molar_ratio(ratio_str):
    ...
    Official order in LNP Atlas is:
    ionizable lipid, PEG-lipid, sterol lipid, helper lipid.
...
def main():
    ...
    # Preserve the raw Atlas values in their official order:
    # ionizable : PEG : sterol : helper
    df["ratio_raw_ionizable"] = [r[0] for r in ratios]
    df["ratio_raw_peg"] = [r[1] for r in ratios]
    df["ratio_raw_sterol"] = [r[2] for r in ratios]
    df["ratio_raw_helper"] = [r[3] for r in ratios]
    ...
    df["ratio_sum"] = df[raw_ratio_cols].sum(axis=1, min_count=4)
    df["ratio_format_valid"] = df[raw_ratio_cols].notna().all(axis=1)
    df["ratio_sum_near_100"] = (
        df["ratio_format_valid"]
        & np.isclose(df["ratio_sum"], 100.0, atol=1.0)
    )

    # Normalize valid positive ratios to percentages summing to 100.
    # This avoids incorrectly assuming every published ratio was already
    # reported as a literal mole percentage.
    valid_sum = df["ratio_sum"].where(df["ratio_sum"] > 0)

    df["mol_pct_ionizable"] = (
        100 * df["ratio_raw_ionizable"] / valid_sum
    )
    df["mol_pct_peg"] = (
        100 * df["ratio_raw_peg"] / valid_sum
    )
    df["mol_pct_sterol"] = (
        100 * df["ratio_raw_sterol"] / valid_sum
    )
    df["mol_pct_helper"] = (
        100 * df["ratio_raw_helper"] / valid_sum
    )

    n_ratio_parsed = int(df["ratio_format_valid"].sum())
    n_near_100 = int(df["ratio_sum_near_100"].sum())

    print(f"  Parsed molar ratio for {n_ratio_parsed}/{len(df)} rows")
    print(
        f"  {n_near_100}/{n_ratio_parsed} parsed ratios already summed "
